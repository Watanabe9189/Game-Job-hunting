...
HRESULT CSound::Init(HWND hWnd)
{
	...
	CheckChunkSound(CSound::LABEL::LABEL_BGM_APPROACH);
...
//終了処理
//<=============================================================================
HRESULT CSound::InitAll(HWND hWnd)
{
	HRESULT hr;
...
HRESULT CSound::CheckChunkSound(const int Label)
{
		...
		&&m_aSizeAudio[Label] == NULL)
	{
		...
		// サウンドデータファイルの生成
		hFile = CreateFile(m_aSoundInfo[Label].pFilename, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, 0, NULL);
		...
		// オーディオデータ読み込み
		hr = CheckChunk(hFile, 'atad', &pSize, &dwChunkPosition);
		...
		m_apDataAudio[Label] = pData;
		m_aSizeAudio[Label] = pSize;

		// ソースボイスの生成
		hr = m_pXAudio2->CreateSourceVoice(&m_apSourceVoice[Label], &(wfx.Format));
		if (FAILED(hr))
		{
